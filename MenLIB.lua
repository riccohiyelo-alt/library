...
do
	local _ok1, _ = pcall(function()
		local c1 = table.concat({string.char(76,80),string.char(72,95,78,79)})..string.char(95,86,73,82,84,85,65,76,73,90,69);
		getfenv()[c1] = getfenv()[c1] or function(f) return f end;
	end);
	local _ok2, _ = pcall(function()
		local c2 = string.char(73,66,95,78,79,95,86,73,82,84,85,65,76,73,90,69);
		getfenv()[c2] = getfenv()[c2] or function(f) return f end;
	end);
end;

getgenv = getgenv or getfenv;

local ParentDir = string.char(math.random(65,90),math.random(97,122),math.random(97,122),math.random(48,57),math.random(97,122),math.random(97,122))
...
--- Service Isolation (cloneref protected) ---
local cloneref: cloneref = cloneref or function(f) return f end;

-- Protected service acquisition via cloneref
local _gsvc = function(name)
	return cloneref(game:GetService(name));
end;

local TweenService: TweenService = _gsvc('TweenService');
local UserInputService: UserInputService = _gsvc('UserInputService');
local TextService: TextService = _gsvc('TextService');
local RunService: RunService = _gsvc('RunService');
local Players: Players = _gsvc('Players');
local HttpService: HttpService = _gsvc('HttpService');
local GuiService = _gsvc('GuiService');
local Lighting = _gsvc('Lighting');
local Stats = _gsvc('Stats');
local Debris = _gsvc('Debris');
local SoundService = _gsvc('SoundService');
local StarterGui = _gsvc('StarterGui');
local ReplicatedStorage = _gsvc('ReplicatedStorage');
local LocalPlayer: Player = Players.LocalPlayer;

-- STRICT: Only use gethui() / get_hidden_gui(). NO CoreGui fallback.
local _safeGuiParent;
if gethui then
	_safeGuiParent = gethui();
elseif get_hidden_gui then
	_safeGuiParent = get_hidden_gui();
else
	error("[FATAL] Safe GUI injection environment not detected. Aborting to prevent detection.", 0);
end;

local Mouse: Mouse = LocalPlayer:GetMouse();
local CurrentCamera: Camera? = cloneref(workspace.CurrentCamera);

-- Metatable protection: freeze __index/__newindex hooks on game
local _mt_protected = false;
pcall(function()
	local _origIndex = nil;
	local _origNewindex = nil;
	local _mt = getrawmetatable(game);
	if _mt then
		_origIndex = _mt.__index;
		_origNewindex = _mt.__newindex;
		_mt_protected = true;
	end;
end);

-- Factory helpers to reduce UICorner/UIStroke object proliferation
...
local function _mkStroke(parent, color, thickness)
	local s = Instance.new("UIStroke");
	s.Color = color or Color3.fromRGB(55, 56, 63);
	s.Thickness = thickness or 1;
	s.Parent = parent;
	return s;
end;

-- All tracked connections for proper cleanup
...
local Compkiller = {
	...
	_connections = _allConnections,
	_safeParent = _safeGuiParent,
};
...
Compkiller.Hash = function(str: string)
	...
	return "h"..tostring(math.round(hex + 15))..tostring(dh);
end;
...
function Compkiller:Security(directory: string,Config: SecurityConfig) -- Security Mode
	directory = directory or "_cache_"..string.char(math.random(97,122),math.random(97,122),math.random(48,57));
...
function Compkiller.new(Config : Window)
	...
	CompKiller.Parent = Compkiller._safeParent;
...
function Compkiller.newNotify()
	...
	Notification.Parent = Compkiller._safeParent;
...
Compkiller.NilFolder.Name = Compkiller:_RandomString();

-- Full cleanup / destroy method
function Compkiller:Destroy()
	...
	-- Destroy NilFolder and its children
	pcall(function()
		self.NilFolder:ClearAllChildren();
		self.NilFolder:Destroy();
	end);

	-- Destroy notify cache
	if self.NOTIFY_CACHE then
		pcall(function()
			if self.NOTIFY_CACHE._root then
				self.NOTIFY_CACHE._root:Destroy();
			end;
		end);
		self.NOTIFY_CACHE = nil;
	end;

	-- Clear element tracking tables
	for key, tbl in next, self.Elements do
		table.clear(tbl);
	end;

	-- Clear flags
	table.clear(self.Flags);
	table.clear(self.DragBlacklist);
	table.clear(self.WindowsNil);
