...
-- variables
	...
	local http_service = cloneref(game:GetService("HttpService"))-- REASON: Dumbass customer put their library in a request and flexed his non existant security and ended up getting it leaked by himself... 😭
-- The code here is horrendous this is my 2nd library, the added on code was made to suit the old code however I should have just converted to a newer version of my code kind of an oopsie. 
	...
	-- elements 
		...
		function library:window(properties)
			...
			-- theming 
				...
				task.spawn(function()
					while task.wait(1) do 
						watermark.change_text(os.date('NeverPaste - ALPHA - %b %d %Y - %H:%M:%S'))
					end 
				end) 
		...
		function library:esp_preview(properties)
			local cfg = {items = {}, objects = {};}

			flags["Name_Color"] = flags["Name_Color"] or {Color = rgb(255, 255, 255)}
			flags["Box_Color"] = flags["Box_Color"] or {Color = rgb(255, 255, 255)}
			flags["Health_Low"] = flags["Health_Low"] or {Color = rgb(255, 0, 0)}
			flags["Health_High"] = flags["Health_High"] or {Color = rgb(0, 255, 0)}
			flags["Distance_Color"] = flags["Distance_Color"] or {Color = rgb(255, 255, 255)}
			flags["Weapon_Color"] = flags["Weapon_Color"] or {Color = rgb(255, 255, 255)}

			local items = cfg.items; do 
				items.preview = library:create("ViewportFrame", {
					Parent = self.holder;
					Name = "\0";
					BackgroundTransparency = 1;
					Size = dim2(1, 0, 0, 220);
				...
				items.preview.CurrentCamera = items.camera

				local function pick_preview_object(obj)
					if not obj then return nil end
					if obj:IsA("Model") or obj:IsA("BasePart") then
						return obj
					end

					local model = obj:FindFirstChildWhichIsA("Model", true)
				...
				local preview_obj
				local ok, objects = pcall(game.GetObjects, game, "rbxassetid://100857536142875")
				if ok and type(objects) == "table" then
					for _, obj in next, objects do
						preview_obj = pick_preview_object(obj)
						if preview_obj then break end
					end
				end

				if preview_obj then
					if preview_obj:IsA("BasePart") then
						local model = Instance.new("Model")
						preview_obj.Parent = model
						preview_obj = model
					end

					preview_obj.Parent = items.preview

					local primary = preview_obj:FindFirstChild("HumanoidRootPart")
						or preview_obj:FindFirstChild("UpperTorso")
						or preview_obj:FindFirstChild("Torso")
						or preview_obj:FindFirstChildWhichIsA("BasePart", true)
					if primary then
						preview_obj.PrimaryPart = primary
					end

					local ok_bb, cf, size = pcall(function()
						return preview_obj:GetBoundingBox()
					end)

					if ok_bb then
						preview_obj:PivotTo(cfr(0, 0, 0))
						local max_dim = math.max(size.X, size.Y, size.Z)
						local distance = max_dim > 0 and (max_dim * 1.6) or 6
						items.camera.CFrame = cfr(vec3(0, 0, distance), vec3(0, 0, 0))
					else
						items.camera.CFrame = cfr(vec3(0, 0, 6), vec3(0, 0, 0))
					end
				else
					items.preview_fallback = library:create("ImageLabel", {
						Parent = items.preview;
						Name = "\0";
						BackgroundTransparency = 1;
						Size = dim2(1, 0, 1, 0);
